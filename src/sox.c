...
static size_t *user_effargs_size = NULL;
...
static size_t *nuser_effects = NULL;
...
static sox_bool no_dither, reported_sox_opts;
  ...
  if (file_count) {
    if (ofile->ft) {
      if (!success && ofile->ft->io_type == lsx_io_file) {
        ...
        if (
          !stat(ofile->ft->filename, &st) && (st.st_mode & S_IFMT) == S_IFREG
        ) {
...
static void play_file_info(sox_format_t *ft, file_t *f, sox_bool full)
{
  ...
  if (ft->filename[0]) {
    ...
    if (
      strcmp(ft->filename, "-") == 0 || (ft->handler.flags & SOX_FILE_DEVICE)
    ) {
  ...
  fprintf(
    output,
    "  Encoding: %-14s", sox_encodings_info[ft->encoding.encoding].name
  );
  ...
  sprintf(
    buffer,
    "  Channels: %u @ %u-bit", ft->signal.channels, ft->signal.precision
  );
...
static void display_file_info(sox_format_t *ft, file_t *f, sox_bool full)
{
  ...
  soxdbg_fenter();
  {
    ...
    fprintf(
      output,
      "\n%s: '%s'",
      ft->mode == 'r' ? "Input File     " : "Output File    ",
      ft->filename
    );
    if (filetype)
      for (i = 0; ft->handler.names[i] && show_type; ++i)
    ...
    if (full) {
      if (
        ...
        || (ft->handler.flags & SOX_FILE_ENDIAN)
      )
        fprintf(
          output,
          "Endian Type    : %s\n",
    ...
    for (ws = 0; ws < olen; ++ws) {
      ...
      if (combine_method == sox_mix || combine_method == sox_mix_power) {
          ...
          *p = 0;
          for (i = 0; i < input_count; ++i)
            if (ws < z->ilen[i] && s < files[i]->ft->signal.channels) {
              ...
              double sample = *p + (double)z->ibuf[i][
                ws * files[i]->ft->signal.channels + s
              ];
        ...
        for (s = 0; s < effp->in_signal.channels; ++s, ++p) {
          ...
          i = 0;
          *p = (
            ws < z->ilen[i] && s < files[i]->ft->signal.channels
            ? z->ibuf[i][ws * files[i]->ft->signal.channels + s]
            : 0
          );
          for (++i; i < input_count; ++i) {
            double sample = *p * (-1. / SOX_SAMPLE_MIN) * (
              ws < z->ilen[i] && s < files[i]->ft->signal.channels
              ? z->ibuf[i][ws * files[i]->ft->signal.channels + s]
              : 0
            );
...
static void auto_effect(
  sox_effects_chain_t *, char const *, int, char **, sox_signalinfo_t *, int *
);

static int add_effect(
...
)
{
  ...
  switch (*guard) {
    ...
    case 1:
      ...
      break;
    case 2:
      if (!(effp->handler.flags & SOX_EFF_MODIFY)) {
        lsx_warn(
  ...
  r = sox_add_effect(chain, effp, in, out);
  soxdbg_fleave();
  return r;
}
...
)
{
  ...
  if (
    add_effect(chain, effp, signal, &ofile->ft->signal, guard) != SOX_SUCCESS
  ) {
...
static void parse_effects(int argc, char **argv)
{
  ...
  while (optstate.ind < argc) {
    ...
    if (eff_offset == user_effargs_size[eff_chain_count]) {
      ...
      lsx_revalloc(
        user_effargs[eff_chain_count], user_effargs_size[eff_chain_count]
      );
    ...
    user_effargs[eff_chain_count][eff_offset].name = lsx_strdup(
      argv[optstate.ind]
    );
    optstate.ind++;
    for (
      j = 0;
      j < (size_t)(argc - optstate.ind)
      && !sox_find_effect(argv[optstate.ind + j])
      && !is_pseudo_effect(argv[optstate.ind + j]);
      ++j
    ) {
      if (j >= user_effargs[eff_chain_count][eff_offset].argv_size) {
        user_effargs[eff_chain_count][eff_offset].argv_size += EFFARGS_STEP;
        lsx_revalloc(
          user_effargs[eff_chain_count][eff_offset].argv,
          user_effargs[eff_chain_count][eff_offset].argv_size
        );
      }
      user_effargs[eff_chain_count][eff_offset].argv[j] = lsx_strdup(
        argv[optstate.ind + j]
      );
    }
...
static void read_user_effects(char const *filename)
{
  ...
  soxdbg_fenter();
  {
    ...
    while (fgets(s + pos, (int)(buffer_size - pos), file)) {
      ...
      if (len && s[pos + len - 1] == '\n')
        ...
        if (ferror(file))
          ...
          break;
        lsx_fail(
          "Error reading effects file `%s' (not a text file?)", filename
        );
        soxdbg_exit(1);
        exit(1);
      }
    ...
    if (ferror(file)) {
      lsx_fail(
        "Error reading effects file `%s': %s", filename, strerror(errno)
      );
      soxdbg_exit(1);
      exit(1);
    }
...
static void create_user_effects(void)
{
  ...
  for (i = 0; i < num_effects; i++) {
    effp = sox_create_effect(sox_find_effect(
      user_effargs[current_eff_chain][i].name
    ));

    if (effp->handler.flags & SOX_EFF_DEPRECATED)
      lsx_warn(
        "effect `%s' is deprecated; see sox(1) for an alternative",
        effp->handler.name
      );
    ...
    if (sox_effect_options(
      effp, user_effargs[current_eff_chain][i].argc,
      user_effargs[current_eff_chain][i].argv
    ) == SOX_EOF) {
      soxdbg_exit(1);
      exit(1);
    }
...
static void add_effects(sox_effects_chain_t *chain)
{
  ...
  for (
    i = 0;
    i < nuser_effects[current_eff_chain]
    && strcmp(user_efftab[i]->handler.name, "dither");
    i++
  ) {
    if (add_effect(
      chain, user_efftab[i], &signal, &ofile->ft->signal, &guard
    ) != SOX_SUCCESS) {
      soxdbg_exit(2);
  ...
  if (
    signal.channels < ofile->ft->signal.channels
  ...
  if (
    i == nuser_effects[current_eff_chain]
    && !no_dither
    && signal.precision > ofile->ft->signal.precision
    && ofile->ft->signal.precision < 24
  )
  ...
  if (!save_output_eff) {
    ...
    if (sox_add_effect(
      chain, effp, &signal, &ofile->ft->signal
    ) != SOX_SUCCESS) {
      soxdbg_exit(2);
      exit(2);
    }
...
static int advance_eff_chain(void)
{
  ...
  if (input_eof)
    ...
    while (
      nuser_effects[current_eff_chain] == 1
...
static uint64_t total_clips(void)
{
  ...
  r = clips + mixing_clips + sox_effects_clips(effects_chain);
  soxdbg_fleave();
  return r;
}
...
static sox_bool since(struct timeval *then, double secs, sox_bool always_reset)
{
  ...
  ret = (
    d > ceil(secs)
    || now.tv_usec - then->tv_usec + d * TIME_FRAC >= secs * TIME_FRAC
  );
  if (ret || always_reset)
    *then = now;
  soxdbg_fleave();
...
static void display_status(sox_bool all_done)
{
  ...
  if (all_done || since(&then, .1, sox_false)) {
    ...
    fprintf(
      ...
      vu(1),
      headroom(),
      lsx_sigfigs3((double)total_clips())
    );
  }
  if (all_done)
    fputc('\n', stderr);
...
static void adjust_volume(int delta)
{
  ...
  int vol1 = 0, vol2 = 0, fd = open(
    from_env ? from_env : "/dev/mixer", O_RDWR
  );
...
static void adjust_volume(int delta)
{
  ...
  if (fd >= 0) {
    ...
    if (ioctl(fd, AUDIO_GETINFO, &audio_info) >= 0) {
      vol1 = (
        (audio_info.play.gain * 100 + (AUDIO_MAX_GAIN >> 1)) / AUDIO_MAX_GAIN
      );
...
static int update_status(sox_bool all_done, void *client_data)
{
  ...
  if (interactive)
    while (kbhit()) {
      ...
      if (
        files[current_input]->ft->handler.seek
        && files[current_input]->ft->seekable
      ) {
        if (ch == '>') {
          /* 30 sec. */
          uint64_t jump = files[current_input]->ft->signal.rate * 30;
          ...
          if (
            ...
            sox_seek(
              files[current_input]->ft, read_wide_samples, SOX_SEEK_SET
            );
        ...
        if (ch == '<') {
          ...
          read_wide_samples = (
            jump < read_wide_samples ? read_wide_samples - jump : 0
          );
          sox_seek(files[current_input]->ft, read_wide_samples, SOX_SEEK_SET);
          /* FIXME: Do something if seek fails. */
        }
      }
      if (ch == 'R') {
...
static void optimize_trim(void)
{
  ...
  if (
    ...
    if (files[0]->ft->handler.seek && files[0]->ft->seekable) {
      ...
      if (
        offset && sox_seek(files[0]->ft, offset, SOX_SEEK_SET) == SOX_SUCCESS
      ) {
...
static sox_bool overwrite_permitted(char const *filename)
{
  ...
  do {
    fprintf(stderr, "%s sox: overwrite `%s' (y/n)? ", myname, filename);
  } while (scanf(" %c%*[^\n]", &c) != 1 || !strchr("yYnN", c));
  soxdbg_fleave();
  return c == 'y' || c == 'Y';
}
...
static void open_output_file(void)
{
  ...
  if (show_progress == sox_option_default)
    show_progress = (
      (ofile->ft->handler.flags & SOX_FILE_DEVICE) != 0
...
static void calculate_combiner_signal_parameters(void)
{
  ...
  for (i = 0; i < input_count; i++) {
    ...
    for (
      j =0;
      j < nuser_effects[current_eff_chain] && !files[i]->ft->signal.channels;
      ++j
    )
  ...
  if (combine_method == sox_sequence) {
    ...
    for (i = 0; i < input_count; i++) {
      ...
      max_length_ws = (
        files[i]->ft->signal.length
        ? max(
            max_length_ws,
            files[i]->ft->signal.length / files[i]->ft->signal.channels
          )
        : SOX_UNKNOWN_LEN
      );
    ...
    if (combine_method == sox_concatenate)
      ...
      combiner_signal.length = (
        max_length_ws != SOX_UNKNOWN_LEN
        ? max_length_ws * combiner_signal.channels
        : SOX_UNKNOWN_LEN
      );
  }
  soxdbg_fleave();
...
static void calculate_output_signal_parameters(void)
{
  ...
  for (i = 0; i < nuser_effects[current_eff_chain] && !ofile->signal.rate; ++i)
    ofile->signal.rate = (
      user_efftab[nuser_effects[current_eff_chain] - 1 - i]->out_signal.rate
    );
  for (
    i = 0; i < nuser_effects[current_eff_chain] && !ofile->signal.channels; ++i
  )
    ofile->signal.channels = user_efftab[
      nuser_effects[current_eff_chain] - 1 - i
    ]->out_signal.channels;
  ...
  for (i = 0; i < nuser_effects[current_eff_chain]; i++)
    known_length = (
      known_length && !(user_efftab[i]->handler.flags & SOX_EFF_LENGTH)
    );

  if (!known_length)
    olen = 0;
  ofile->signal.length = (uint64_t)(
    olen * ofile->signal.channels * ofile->signal.rate / combiner_signal.rate
    + .5
  );
  soxdbg_fleave();
}
...
static int process(void)
{
  ...
  if (!effects_chain)
    effects_chain = sox_create_effects_chain(
      &combiner_encoding, &ofile->ft->encoding
    );
  ...
  for (i = 0; sox_effect_fns[i]; i++) {
    ...
    if (e && e->name)
      printf(
        " %s%s",
        e->name,
        (e->flags & SOX_EFF_DEPRECATED)
        ? "*"
        : (e->flags & SOX_EFF_ALPHA)
          ? "+"
...
static void usage(char const *message)
{
  soxdbg_fenter();
  {
    ...
    static char const * const lines1[] = {
      ...
      "-                        Pipe/redirect input/output (stdin/stdout);"
      " may need -t",
    ...
    static char const * const lines2[] = {
      ...
      "--help-effect NAME       Show usage of effect NAME, or NAME=all for"
      " all",
    ...
    static char const * const lines3[] = {
      ...
      " `play'",
      "--plot gnuplot|octave    Generate script to plot response of filter"
      " effect",
      "-q, --no-show-progress   Run in quiet mode; opposite of -S",
      "--replay-gain track|album|off  Default: off (sox, rec), track (play)",
      "-R                       Use default random numbers (same on each run"
      " of SoX)",
      "-S, --show-progress      Display progress while processing audio data",
      "--single-threaded        Disable parallel effects channels processing",
      "--temp DIRECTORY         Specify the directory to use for temporary"
      " files",
      "-T, --combine multiply   Multiply samples of corresponding channels"
      " from all",
      ...
      " and not",
      "overridden by any of various means including providing output format"
      " options.",
      "",
      "-v|--volume FACTOR       Input file volume adjustment factor (real"
      " number)",
      "--ignore-length          Ignore input file length given in header; read"
      " to EOF",
      ...
      "--endian little|big|swap Encoded byte-order; swap means opposite to"
      " default",
      ...
      "--comment-file FILENAME  File containing comment text for the output"
      " file",
#if HAVE_GLOB_H
...
static void usage_format1(sox_format_handler_t const *f)
{
  ...
  if (f->flags & SOX_FILE_CHANS) {
    ...
    if (f->flags & SOX_FILE_MONO)
      printf(" mono");
    if (f->flags & SOX_FILE_STEREO)
      printf(" stereo");
    if (f->flags & SOX_FILE_QUAD)
      printf(" quad");
    putchar('\n');
  }
  if (f->write_rates) {
...
static void read_comment_file(
  sox_comments_t *comments, char const * const filename
)
{
  int c;
...
static char const * const getoptstr = "+b:c:de:hmnpqr:t:v:xBC:DGLMNRSTV::X";
...
static int enum_option(
  char const *arg, int option_index, lsx_enum_item const *items
)
{
  lsx_enum_item const *r;
...
#define SOX_BUFMIN 16
              if (
                sscanf(optstate.arg, "%i %c", &i, &dummy) != 1
                || i <= SOX_BUFMIN
              ) {
                lsx_fail(
                  "Buffer size `%s' must be > %d", optstate.arg, SOX_BUFMIN
                );
                soxdbg_exit(1);
                exit(1);
              }
              ...
              break;
            case 2:
              combine_method = enum_option(
                optstate.arg, optstate.lngind, combine_methods
              );
              break;
            case 3:
            ...
            case 5:
              if (
                f->encoding.reverse_bytes != sox_option_default
                || f->encoding.opposite_endian
              )
                usage("only one endian option per file is allowed");
              switch (
                enum_option(optstate.arg, optstate.lngind, endian_options)
              ) {
                case ENDIAN_little:
                  f->encoding.reverse_bytes = MACHINE_IS_BIGENDIAN;
                  break;
                case ENDIAN_big:
                  f->encoding.reverse_bytes = MACHINE_IS_LITTLEENDIAN;
                  break;
                case ENDIAN_swap:
...
#else
              lsx_fail(
                "Interactive mode has not been enabled at compile time."
              );
              soxdbg_exit(1);
              exit(1);
              break;
            ...
            case 10:
              ...
              break;
            case 11:
              sox_effects_globals.plot = enum_option(
                optstate.arg, optstate.lngind, plot_methods
              );
              break;
            case 12:
              replay_gain_mode = enum_option(
                optstate.arg, optstate.lngind, rg_modes
              );
              break;
            case 13:
              display_SoX_version(stdout);
            ...
            case 14:
              break;
            case 15:
              effects_filename = lsx_strdup(optstate.arg);
              break;
            case 16:
              sox_globals.tmp_path = lsx_strdup(optstate.arg);
              break;
            case 17:
              sox_globals.use_threads = sox_false;
              break;
            case 18:
            ...
            case 20:
              ...
              break;
            case 21:
              play_rate_arg = lsx_strdup(optstate.arg);
              break;
            case 22:
            ...
            case 23:
              ...
              break;
            case 24:
              sox_globals.use_threads = sox_true;
              break;
            case 25:
              if (
                sscanf(optstate.arg, "%i %c", &i, &dummy) != 1
        ...
        case 'r':
          {
            char k = 0;
            size_t n = sscanf(
              optstate.arg, "%lf %c %c", &f->signal.rate, &k, &dummy
            );

            if (
              n < 1 || f->signal.rate <= 0 || (n > 1 && k != 'k') || n > 2
            ) {
              lsx_fail(
                "Rate value `%s' is not a positive number", optstate.arg
              );
              soxdbg_exit(1);
              exit(1);
            }
        ...
        case 'c':
          if (sscanf(optstate.arg, "%d %c", &i, &dummy) != 1 || i <= 0) {
            lsx_fail(
              "Channels value `%s' is not a positive integer", optstate.arg
            );
            soxdbg_exit(1);
            exit(1);
          }
        ...
        case 'C':
          if (sscanf(
            optstate.arg, "%lf %c", &f->encoding.compression, &dummy
          ) != 1) {
        ...
        case 'b':
          if (sscanf(optstate.arg, "%d %c", &i, &dummy) != 1 || i <= 0) {
            lsx_fail(
              "Bits value `%s' is not a positive integer", optstate.arg
            );
            soxdbg_exit(1);
            exit(1);
          }
        ...
        case 'V':
          if (optstate.arg == NULL)
            ...
            if (sscanf(optstate.arg, "%d %c", &i, &dummy) != 1 || i < 0) {
              ...
              lsx_fail(
                "Verbosity value `%s' is not a non-negative integer",
                optstate.arg
              );
              soxdbg_exit(1);
              exit(1);
...
#    define GLOB_TILDE 0
#  endif
static int add_glob_file(
  file_t const * const opts, char const * const filename
)
{
  glob_t globbuf;
  size_t i;
...
static void parse_options_and_filenames(int argc, char **argv)
{
  soxdbg_fenter();
  {
    ...
    for (
      ;
      optstate.ind < argc && !sox_find_effect(argv[optstate.ind]);
      init_file(&opts)
    ) {
      ...
      else if (sox_parse_playlist(
        (sox_playlist_callback_t)add_file, &opts, argv[optstate.ind++]
      ) != SOX_SUCCESS) {
        soxdbg_exit(1);
        exit(1);
      }
    ...
    if (sox_mode == sox_play)
      add_file(&opts, set_default_device(&opts));
    else if (memcmp(&opts, &opts_none, sizeof(opts)))
      /* fopts but no file */
  ...
  soxdbg_fenter();
  {
    ...
    switch (*type) {
      ...
      case Annotation:
        ...
        break;
      case Full:
        display_file_info(ft, NULL, sox_false);
        break;
    }
    r = !!sox_close(ft);
    soxdbg_fleave();
...
static int soxi(int argc, char * const *argv)
{
  ...
  if (argc < 2)
    soxi_usage(0);
  lsx_getopt_init(
    argc, argv, opts, NULL, lsx_getopt_flag_opterr, 1, &optstate
  );
    ...
    if (opt == 'V') {
      ...
      if (optstate.arg == NULL)
        ...
        if (sscanf(optstate.arg, "%d %c", &i, &dummy) != 1 || i < 0) {
          sox_globals.verbosity = 2;
          lsx_fail(
            "Verbosity value `%s' is not a non-negative integer", optstate.arg
          );
          soxdbg_exit(1);
          exit(1);
        }
  ...
  for (; optstate.ind < argc; ++optstate.ind) {
    if (sox_is_playlist(argv[optstate.ind]))
      num_errors += (sox_parse_playlist(
        (sox_playlist_callback_t)soxi1, &type, argv[optstate.ind]
      ) != SOX_SUCCESS);
    else
...
int main(int argc, char **argv)
{
  ...
  if (file_count) {
    sox_format_handler_t const * handler = sox_write_handler(
      ofile->filename, ofile->filetype, NULL
    );
    is_player = (
      handler
  ...
  if (ofile->volume != HUGE_VAL)
    usage(
      "-v can be given only for an input file;\n"
  ...
  for (i = 0; i < input_count; i++) {
    ...
    files[j]->ft = sox_open_read(
      f->filename, &f->signal, &f->encoding, f->filetype
    );
    if (!files[j]->ft) {
  ...
  /* Not the best way for users to do this; now deprecated in favour of
   * soxi.
   */
  if (
    !show_progress
    && !nuser_effects[current_eff_chain]
    && ofile->filetype
    && !strcmp(ofile->filetype, "null")
  ) {
    for (i = 0; i < input_count; i++)
      report_file_info(files[i]);
    soxdbg_exit(0);
  ...
  if (mixing_clips > 0)
    lsx_warn(
      "mix-combining clipped %" PRIu64 " samples; decrease volume?",
      mixing_clips
    );
  ...
  for (i = 0; i < file_count; i++)
    if (files[i]->volume_clips > 0)
      lsx_warn(
        "`%s' balancing clipped %" PRIu64 " samples; decrease volume?",
        files[i]->filename,
